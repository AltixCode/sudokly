...
describe("Paywall", () => {
  ...
  // The pull-quote is the page's visual anchor: its kicker label and the
  // masthead headline above it both have to render as real text, not just
  // the quoted promise itself.
  it("leads with the masthead headline and the promise pull-quote", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("paywallTitle"))).toBeTruthy();
  ...
  it("renders no paragraph for a claim this app does not make", async () => {
